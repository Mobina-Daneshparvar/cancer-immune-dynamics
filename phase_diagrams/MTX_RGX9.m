...
% ----------------------------------------------------------
% Description:
% Simulates cell dynamics under combined MTX and RGX treatment.
% Including:
...
g = 6.4588;             % RGX-induced MDSC death rate
j = [1.22, 0.00005];    % j(1): MTX-induced death for C− and C+ cells (μ), j(2): C+ → Cp+ conversion (ꞷ)
...
dp(2) = a1 * p(2) * (1 - C_tot/C_max) ...
      - b(1)*p(2) + b(2)*p(1) ...
      + epsilon*(c_m/(r + c_m))*c_t*p(1) ...
      - t * p(3) * p(2)- j(1) * p(2) - j(2) * p(2);  % C+

dp(3) = s + p(3)*p(6)*(C_tot^2 / (k + C_tot^2)) ...
      - n * p(3) ...
      - o * p(3) * p(4);                             % I

dp(4) = p(7) * C_tot - B * p(4) - g * p(4);          % M

dp(5) = a2 * p(5) * (1 - C_tot/C_max) ...
      - t * p(3) * p(5) + j(2) * p(2);               % Cp+

dp(6) = n * (0 - p(6));                              % q

dp(7) = B * (0 - p(7));                              % alpha_m
